...
class EquiposController {
    ...
    //actualizar por id
    async actualizarEquipoId({ params, request, response }: { params: any, request: any, response: any }) {
    ...
    async eliminarEquipoId({params, response }: { params: any; response: any }) {
        const codigo = params.codigo
        const result = await PgDatabase.query(`DELETE FROM equipo WHERE codigo = $1`,
            [codigo]
        )
